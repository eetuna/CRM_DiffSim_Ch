function ParameterOptions = DefinitionOfCatheterAndMagneticField_prototype_6b(youngModulus,shearModulus,Alignment_Parameters)
...
marker_length_base = 6.40;%5.50
marker_length_coil = 19.10; %18.3;
marker_length_tip = 6.00; %5.62
...
NumSegments = 8;
L = 82.65;  %catheter length (mm)  82.00
Delta = L/NumSegments;%(mm)
...
C = 1/Delta*[E*(I_x*I_y-I_xy^2) 0 0;
             ...
             0 0 G*J];%(N*mm^5) &(N*mm)
C_lastSeg = 1/L_A*[E*(I_x*I_y-I_xy^2) 0 0;
                   0 E*(I_x*I_y-I_xy^2) 0;
                   0 0 G*J];%(N*mm^5) &(N*mm)
...
W_A_x = pi*(r1^2-r2^2)*L_A*(rho_cath-rho_blood)*g*1.0e-9; %Unit length weight (N)
W_A = [0 0 W_A_x]';
...
MuMatrix = zeros(3,3);
MuMatrix(1,1) = 0.5*N_1*A_1*(cos(theta_1_a)*cos(theta_2_a) + cos(theta_1_b)*cos(theta_2_b));
MuMatrix(2,1) = 0.5*N_1*A_1*(sin(theta_2_a) + sin(theta_2_b));
MuMatrix(3,1) = 0.5*N_1*A_1*(-sin(theta_1_a)*cos(theta_2_a) - sin(theta_1_b)*cos(theta_2_b));

MuMatrix(1,2) = -0.5*N_2*A_2*(sin(alpha_2_a) + sin(alpha_2_b));
MuMatrix(2,2) = 0.5*N_2*A_2*(cos(alpha_1_a)*cos(alpha_2_a) + cos(alpha_1_b)*cos(alpha_2_b));
MuMatrix(3,2) = 0.5*N_2*A_2*(sin(alpha_1_a)*cos(alpha_2_a) + sin(alpha_1_b)*cos(alpha_2_b));

MuMatrix(1,3) = N_3*A_3*(sin(beta_2));
MuMatrix(2,3) = -N_3*A_3*(sin(beta_1)*cos(beta_2));
MuMatrix(3,3) = N_3*A_3*(cos(beta_1)*cos(beta_2));
