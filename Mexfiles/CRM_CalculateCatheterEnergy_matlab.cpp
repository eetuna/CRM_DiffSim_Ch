...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    double *deltau0;
    deltau0 = mxGetDoubles(prhs[0]);

    double *ftip;
    ftip = mxGetDoubles(prhs[1]);

    double *control_inputs;
    control_inputs = mxGetDoubles(prhs[2]);

    const mxArray* CParamStruct = mxGetField(prhs[3], 0, "CathParams");
    const mxArray* CConfigStruct = mxGetField(prhs[3], 0, "CathConfig");
    ...
    */
    ...
    // we only want the final value - no need to calculate the marker locations and coil orientations
	...
	CRMSolverIVP(BVPParams, deltau0, ftip, CalculateEnergy, FinalValueOnly, xf, residual, PotentialEnergy, Params.ReportedMarkerPos, Params.ReportedCoilOrient);


    // --- copy outputs to the lhs structures
    ...
    mxSetPr(plhs[1], dynamicDataforRMP);
