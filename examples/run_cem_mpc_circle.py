...
def main():
    ...
    target_np, dt, li_val = _load_targets("data/dyn_fk_ramp_circle1_hold1.npz")
    targets = torch.tensor(target_np[:81], dtype=torch.float64)
    x0, _ = build_state()
    ...
    if args.plot:
        try:
            ...
            save_tip_trajectory_plot(
                ...
                title="Tip trajectory (CEM circle)",
                out_path=f"{out_dir}/cem_circle_tip_{stamp}",
            )
            save_currents_plot(
                result.u_roll[:, 0].cpu().numpy(),
                title="Currents (CEM circle)",
                out_path=f"{out_dir}/cem_circle_currents_{stamp}",
            )
            save_error_plot(
                errors,
                title="Tip tracking error (CEM circle)",
                out_path=f"{out_dir}/cem_circle_error_{stamp}",
            )
            print(f"Saved plots to {out_dir}/cem_circle_*_{stamp}.png/.pdf")
        except RuntimeError:
