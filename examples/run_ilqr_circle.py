...
def main():
    ...
    target_np, currents_np, dt, li_val = _load_targets("data/dyn_fk_ramp_circle1_hold1.npz")
    ...
    if args.max_wall_sec is not None and time.time() - start_time > args.max_wall_sec:
        ...
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        np.savez(
            f"{out_dir}/ilqr_circle_partial_{stamp}.npz",
            x_roll=x_roll.detach().cpu().numpy(),
            u_seq=u_final.detach().cpu().numpy(),
        )
        if args.profile_step and u_roll.shape[0] > 0:
            run_rollout_profile(x0, u_roll, li, cfg_dyn, label="circle_profile_partial", save_dir=out_dir)
    ...
    if any(s.get("nan_detected") for s in stats):
        ...
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        np.savez(
            f"{out_dir}/ilqr_circle_nan_{stamp}.npz",
            x_roll=x_roll.detach().cpu().numpy(),
            u_seq=u_final.detach().cpu().numpy(),
            iter_index=0,
            fail_step=-1,
