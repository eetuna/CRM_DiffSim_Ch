...
def _fill_nan_targets(target_np):
    if not np.isnan(target_np).any():
        return target_np
    filled = target_np.copy()
    for dim in range(filled.shape[1]):
        ...
        col[:first] = col[first]
        for idx in range(first + 1, col.shape[0]):
            if not np.isfinite(col[idx]):
                col[idx] = col[idx - 1]
        for idx in range(col.shape[0] - 2, -1, -1):
            if not np.isfinite(col[idx]):
                col[idx] = col[idx + 1]
        filled[:, dim] = col
    return filled
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["hold", "staged", "track", "replay_only_full", "replay_dataset_mode_full"],
        default="hold",
    )
    parser.add_argument("--N-warm", type=int, default=10, help="Hold target for the first N_warm steps")
    parser.add_argument("--N-blend", type=int, default=20, help="Blend hold->lemniscate over N_blend steps")
    ...
    if args.mode == "replay_only_full":
        ...
        args.full_dataset = True
    elif args.mode in {"hold", "staged", "track"}:
        pass
    ...
    dataset_path = "data/dyn_fk_lem1_y40_a10_L94_hold1.npz"
    target_np, currents_np, dt, li_val = _load_targets(dataset_path)
    target_np = _fill_nan_targets(target_np)
    ...
    if args.replay_dataset_mode or args.replay_only:
        ...
        print(
            "mode="
            f"{args.mode}, init_u={args.init_u}, bounds_mode={args.bounds_mode}, Li_mm={li_val:.1f}, "
    ...
    if args.full_dataset:
        ...
        full_targets = torch.tensor(target_np, dtype=torch.float64)
        n_steps = max(0, full_targets.shape[0] - 1)
    ...
    if args.mpc_steps is not None:
        ...
        full_targets = full_targets[: n_steps + 1]

    if args.fast:
        n_steps = min(n_steps, 3)
        full_targets = base_tip.repeat(n_steps + 1, 1)
    ...
    if args.mode == "hold":
        ...
        warm_end = min(args.N_warm, n_steps)
        for t in range(warm_end + 1):
            staged_targets[t] = base_tip
        blend_start = warm_end + 1
        blend_end = min(n_steps, warm_end + max(args.N_blend, 0))
        if args.N_blend > 0 and blend_start <= blend_end:
    ...
    if args.replay_dataset_mode or args.replay_only:
        out_path = f"{out_dir}/replay_mode_lemniscate_{stamp}.npz"
    else:
        out_path = f"{out_dir}/ilqr_lemniscate_{stamp}.npz"
    start_time = time.time()
    last_heartbeat = start_time
    last_result = None
    max_wall_hit = False

    if not args.replay_dataset_mode:
    ...
    def _emit_report(report_errors):
        ...
        report_path = os.path.join(
            "docs/control/run_reports", f"run_ilqr_lemniscate_{stamp}.md"
        )
        write_run_report(
            ...
            plot_prefix=f"{out_dir}/ilqr_lemniscate_{stamp}" if args.plot else None,
    ...
    if args.profile_step:
        if u_roll_cat is not None and u_roll_cat.shape[0] > 0:
            run_rollout_profile(x0, u_roll_cat, li, cfg_dyn, label="lemniscate_profile", save_dir="output_data")
