...
def test_crm_step_B_step_matches_fd():
    ...
    for _ in range(3):
        ...
        B_new = ext.crm_dyn_jacobians(eta, x_t, u_t, li, cfg)[1]
        B_fd = ext.crm_dyn_jacobians_fd(eta, x_t, u_t, li, cfg)[1]

        diff = torch.abs(B_new - B_fd)
        rel = diff / torch.clamp(torch.abs(B_fd), min=1e-8)
    ...
    # B_step_new uses scale-aware steps; expect close match to fixed-step FD.
    assert max_abs <= 1e-5
    assert max_rel <= 1e-4
