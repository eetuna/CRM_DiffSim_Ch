...
def test_dyn_residual_callable():
    ext = _load_ext()
    ...
    residual, res_u0, res_tau = ext.crm_dyn_residual(eta, x_t, u_t, li, cfg)

    res_norm_calc = torch.linalg.vector_norm(residual, dim=1)

    print("eta*", eta)
    print("residual_norm", res_norm)

    assert eta.shape[1] == 6
    assert residual.shape[1] == 6
    assert torch.allclose(res_norm, res_norm_calc, atol=1e-10, rtol=1e-8)
