...
def test_phase_b_full_dataset_acceptance_lemniscate():
    ...
    baseline_npz = _run_and_get_npz(baseline_cmd, "replay_mode_lemniscate_*.npz")
    baseline_mean, baseline_max, baseline_data = _mean_max_error(baseline_npz)
    print(f"lemniscate baseline mean={baseline_mean:.3f} max={baseline_max:.3f}")
    ...
    ctrl_npz = _run_and_get_npz(ctrl_cmd, "replay_mode_lemniscate_*.npz", allow_timeout=True)
    ctrl_mean, ctrl_max, ctrl_data = _mean_max_error(ctrl_npz)
    print(f"lemniscate control mean={ctrl_mean:.3f} max={ctrl_max:.3f}")
    ...
    comparison_path = os.path.join(
        report_dir, f"phase_b_acceptance_lemniscate_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    )
    write_run_report(
        report_path=comparison_path,
        command="; ".join([" ".join(baseline_cmd), " ".join(ctrl_cmd)]),
        dataset_name="data/dyn_fk_lem1_y40_a10_L94_hold1.npz",
        start_idx=0,
        end_idx=150,
        n_total=150,
