...
def test_dyn_jacobians_and_vjp_shapes():
    ext = _load_ext()
    ...
    a_step, b_step, c_step = ext.crm_dyn_jacobians(eta, x_t, u_t, li, cfg)

    assert a_step.shape == (1, 6, 6)
    assert b_step.shape == (1, 6, 3)
    assert c_step.shape == (1, 6, 39)
    assert torch.isfinite(a_step).all()
    assert torch.isfinite(b_step).all()
    assert torch.isfinite(c_step).all()

    g = torch.ones_like(x_tp1)
    vjp_eta, vjp_u, vjp_x = ext.crm_dyn_vjp(g, eta, x_t, u_t, li, cfg)
    assert vjp_eta.shape == (1, 6)
    assert vjp_u.shape == (1, 1, 3)
    assert vjp_x.shape == (1, 39)
    assert torch.isfinite(vjp_eta).all()
    assert torch.isfinite(vjp_u).all()
    assert torch.isfinite(vjp_x).all()
