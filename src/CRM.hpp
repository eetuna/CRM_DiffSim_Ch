...
#define CONTACT_TRUSTREGION_ANALYTICALJAC			//    Use the analytical jacobian in contact trustregion solution 
	...
	public:
		...
		double* SegLengths;					// no_segments element array of Segment Lengths
		double* rho;						// no_segments element array of Length Density (mass per unit length) of the flexible catheter substrate (tubing)
		double* InnerRadius;				// no_flex_seg element array of Inner Radii of the flexible catheter segments
		double* OuterRadius;				// no_flex_seg element array of Outer Radii of the flexible catheter segments
		double* YoungsModulus;				// no_flex_seg element array of Young's Moduli of the flexible catheter segments
		double* ShearModulus;				// no_flex_seg element array of Shear Moduli of the flexible catheter segments
	...
	struct CRMForwardKinematicsData {
		...
		double	TipForce[3];					// force applied at the tip of the catheter in spatial coordinates - only used if ContactMode == ContactModeType::FREE_TIP
		double	deltau0_initialguess[3];		// initial guess for the \delta curvature at the base of the catheter ( u0 = deltau0 + ustar0 )
		...
		double  (*ReportedMarkerPos)[3];		// pointer to an output array where the calculated marker locations will be stored
		double  (*ReportedCoilOrient)[9];		// pointer to an output array where the calculated coil orientations will be stored
...
//      This is a wrapper, which calls the Eigen vectorized version of the CRM_ForwardKinematics()
	int CRM_ForwardKinematics(   // return value ==0 successful; !=0 if algorithms is stuck at a local minimum, or cannot make further progress
		double in_x[],		// array of length (3 x NUM_ACT_SET + 1)  -- the +1 is for the inserted length of the catheter
							...
							//  Currents are in A (Ampere)
							//  Insertion length is in mm
		...
		double& out_PotentialEnergy,		// The calculated potential energy (strain + magnetic + gravitational) of the catheter
